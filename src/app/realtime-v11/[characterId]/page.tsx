...
export default function RealtimeCallPage() {
  ...
  const [currentSpeaker, setCurrentSpeaker] = useState('');   // v11 聲紋：現在說話的人（顯示名/訪客N）
  ...
  const handleConnect = async () => {
    ...
    setErrorMsg(''); setHealth(INITIAL_HEALTH); setDiagLogs([]); setCurrentSpeaker('');
    ...
    try {
      ...
      const tokenRes = await fetch('/api/livekit/token', {
        ...
        body: JSON.stringify({ characterId, v11: true }),
      });
      if (!tokenRes.ok) {
      ...
      room
        ...
        .on(RoomEvent.DataReceived, (payload: Uint8Array) => {
          try {
            ...
            else if (msg.type === 'current_speaker' && msg.name) setCurrentSpeaker(msg.name);
          } catch { /* ignore */ }
        });
    ...
    <div style={{ position:'fixed', inset:0, overflow:'hidden', background:'#111' }}>
      ...
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column',
        ...
        <div style={{ textAlign:'center' }}>
          ...
          <div style={{fontSize:10,color:'rgba(255,255,255,0.2)',marginTop:6,letterSpacing:1}}>v11.0 聲紋辨識 (voiceprint 講者分群)</div>
        </div>

        {/* v11 即時「現在說話」指示器（聲紋分群 → 顯示名/訪客N） */}
        {inCall && currentSpeaker && (
          <div style={{ fontSize:12, color:'rgba(255,255,255,0.5)', letterSpacing:1, marginBottom:2 }}>
            現在說話：{currentSpeaker}
          </div>
        )}
        {/* Latest caption */}
