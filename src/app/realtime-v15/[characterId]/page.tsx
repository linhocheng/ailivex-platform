...
export default function RealtimeCallPage() {
  ...
  const agentIdentityRef = useRef('');  // v15: for RPC destinationIdentity
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const tokenRes = await fetch('/api/livekit/token', {
        ...
        body: JSON.stringify({ characterId, v15: true }),
      });
      if (!tokenRes.ok) {
      ...
      room
        ...
        .on(RoomEvent.ParticipantConnected, (p: RemoteParticipant) => {
          ...
          agentIdentityRef.current = p.identity;  // v15: capture for RPC
  ...
  // v15: send URL to agent via RPC share_source
    ...
    <div style={{ position:'fixed', inset:0, overflow:'hidden', background:'#111' }}>
      ...
      {logOpen && (
        <div className="ax-enter" style={{ position:'absolute', right:18, bottom:120, width:320, maxHeight:260, zIndex:6,
          ...
          <div style={{ fontSize:12.5, fontWeight:600, color:'rgba(255,255,255,0.4)', marginBottom:8 }}>診斷日誌 <span style={{fontSize:10,opacity:0.5}}>v15 讀網址</span></div>
