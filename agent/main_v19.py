"""
ailivex-realtime-agent-v19 — 即時語音 19.0 啟動入口（= v18 + 方法論共創提案工具 propose_method）

v17.4 差異：只加一層 GatedPauseOutput——音量沒提高的聲音不再暫停角色語音
（咳嗽/應和零死空氣）；真打斷 commit 直通，體感與 v17 一致。見 agent/interrupt_gate.py。

與其他版本完全隔離：不同 agent_name（ailivex-realtime-v19）+ 獨立 Cloud Run 服務。
同一 image，啟動命令 override 成：python -m agent.main_v19 start

用法：
  python -m agent.main_v19 dev    # 本地開發
  python -m agent.main_v19 start  # 正式環境（Cloud Run）
"""
import os
from livekit.agents import cli, JobProcess, WorkerOptions
from agent.realtime_agent_v19 import entrypoint, load_vad
...
if __name__ == "__main__":
    ...
    cli.run_app(WorkerOptions(
        ...
        prewarm_fnc=prewarm,
        agent_name="ailivex-realtime-v19",
        port=port,
