...
logger = logging.getLogger("ailivex-realtime-v4")
...
async def entrypoint(ctx: JobContext):
    ...
    if not soniox_key:
        ...
        return
    # v4 單機群聊：開 Soniox diarization（每字帶 speaker 標籤），再用框架內建
    # MultiSpeakerAdapter 把「非主說話者」的逐字稿標出來，讓 LLM 看得到「另一個人」在講。
    base_stt = soniox.STT(
        api_key=soniox_key,
    ...
    stt = MultiSpeakerAdapter(
        stt=base_stt,
        detect_primary_speaker=True,        # 主說話者（拿手機的人）講話照常
        suppress_background_speaker=False,  # 其他人不丟掉、要聽得見
        primary_format="{text}",
    ...
    def _on_user_transcribed(ev):
        # v4 驗證：Soniox diarization 的 speaker_id 有沒有真的流進來、準不準
        if getattr(ev, "is_final", False):
            sid = getattr(ev, "speaker_id", None)
            logger.info(f"v4 STT speaker_id={sid!r} → {getattr(ev, 'transcript', '')[:60]!r}")
