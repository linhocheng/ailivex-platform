...
logger = logging.getLogger("ailivex-realtime-v2")
...
async def entrypoint(ctx: JobContext):
    ...
    # ── 3a spike：主動插話 + 被打斷回饋（1:1 先驗管道）──
    # imThreshold 1-5（越高越主動），暫從 voiceSettings 讀，預設保守 3。
    # 評分(判斷)交 LLM；觸發(靜默/cooldown/gate)用確定性程式擋（天條）。
    im_threshold = get_im_threshold(_conv)
    silence_secs = 4.0 - (im_threshold - 1) * 0.5   # 1→4.0s … 5→2.0s
    cooldown_secs = 12.0
    _interject = {"last": 0.0, "timer": None}

    def _cancel_timer():
        if _interject["timer"] is not None:
            _interject["timer"].cancel()
            _interject["timer"] = None

    async def _maybe_interject():
        _interject["timer"] = None
        # gate：不蓋過人、自己沒在說、過 cooldown
        if session.current_speech is not None:
            return
        if getattr(session, "agent_state", "") == "speaking":
            return
        if time.time() - _interject["last"] < cooldown_secs:
            return
        _interject["last"] = time.time()
        logger.info(f"3a: 靜默 {silence_secs:.1f}s → 評估主動開口 (imThreshold={im_threshold})")
        try:
            # 先便宜判斷「有沒有想說的」→ 有才出聲（避免把「沒有」念出來）
            from anthropic import AsyncAnthropic
            recent = "\n".join(f"{t['role']}: {t['content']}" for t in transcript[-6:]) or "（還沒有對話）"
            _bu = os.environ.get("BRIDGE_URL", "")
            _bs = os.environ.get("BRIDGE_SECRET", "")
            if _bu and _bs:
                _client = AsyncAnthropic(api_key=_bs, base_url=_bu)   # 走 Bridge 吃 Max，不燒額度
            ...
            resp = await _client.messages.create(
                model=_model,
                max_tokens=80,
                system=system_prompt + "\n\n[主動性判斷] 對話冷場了，用你的人格判斷此刻是否自然想主動說一句話。",
                messages=[{"role": "user", "content": (
                    f"最近對話：\n{recent}\n\n"
                    "現在冷場。若你此刻自然、不突兀地想說一句（接話題／關心對方／拋個問題），"
                    "就『只輸出那一句話本身』；若沒有、或硬找話會尷尬，就輸出空字串，什麼都不要寫。"
                )}],
            )
            text = "".join(b.text for b in resp.content if getattr(b, "type", "") == "text").strip()
            text = text.strip("（）()「」\"' ")
            if text and text not in ("沉默", "NOTHING", "無"):
                logger.info(f"3a: 主動說 → {text[:60]!r}")
                session.say(text)   # 走 MiniMax TTS（內含 opencc 繁→簡）
            else:
                logger.info("3a: 評估後選擇沉默")
        except Exception as e:
    ...
    def _arm_timer():
        ...
        _interject["timer"] = loop.call_later(
            silence_secs, lambda: asyncio.create_task(_maybe_interject())
        )
    ...
    def _on_user_state(ev):
        ...
        if new == "speaking":
            _cancel_timer()  # 人在講，取消主動計時
            if getattr(session, "agent_state", "") == "speaking":
                logger.info("3a: 用戶在角色說話時插話 → 讓位（被打斷回饋）")
        elif new == "listening":
            pass  # 3a 主動插話暫關：1:1 幾乎無真空冷場（角色秒接話），留待 P3 群聊再開
            # _arm_timer()

    logger.info(f"3a active: silence_secs={silence_secs:.1f} cooldown={cooldown_secs}s")
