"""
ailivex-realtime-agent-v18 — 即時語音 18.0 啟動入口（= v17 + 優雅讓位 graceful yield：被打斷講完子句才讓位）

v16 差異：prewarm_fnc 預載 VAD + num_idle_processes=1 常備熱行程（接通延遲）
         + VAD 0.3 / TTS 首段 flush（見 realtime_agent_v18.py）

與其他版本完全隔離：不同 agent_name（ailivex-realtime-v18）+ 獨立 Cloud Run 服務。
同一 image，啟動命令 override 成：python -m agent.main_v18 start

用法：
  python -m agent.main_v18 dev    # 本地開發
  python -m agent.main_v18 start  # 正式環境（Cloud Run）
"""
import os
...
if __name__ == "__main__":
    ...
    cli.run_app(WorkerOptions(
        ...
        prewarm_fnc=prewarm,
        agent_name="ailivex-realtime-v18",
        port=port,
