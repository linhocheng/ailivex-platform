"""
ailivex-realtime-agent v5 — v4 + 發話對象偵測（addressee gate）

新增：AilivexAgentV5.on_user_turn_completed
  - 用戶說完一句話，先判斷是不是在叫第三方說話
  - 「接下來請 xxx 說」→ raise StopResponse，AI 靜默讓位
  - 一般對話 → 正常回話

其他機制（Soniox diarization、3a 主動發話、記憶收尾）完全繼承 v4。
"""
import asyncio
import json
import logging
...
logger = logging.getLogger("ailivex-realtime-v5")
...
class AilivexAgentV5(Agent):
    """v4 Agent + 發話對象偵測：用戶把棒子交給第三方時靜默讓位。"""
    ...
    async def on_user_turn_completed(self, turn_ctx, new_message) -> None:
        ...
        if text and is_redirecting_away(text, self._agent_names):
            logger.info(f"v5 gate: 非對 AI → 靜默讓位 {text[:60]!r}")
            raise StopResponse
...
async def entrypoint(ctx: JobContext):
    ...
    agent = AilivexAgentV5(
        instructions=system_prompt,
        tools=[remember_tool, write_document_tool],
        agent_names=agent_names,
    )
    logger.info(f"Agent v5 initialized: soul={len(system_prompt)} chars, agent_names={agent_names}")
    ...
    def _on_user_transcribed(ev):
        if getattr(ev, "is_final", False):
            ...
            logger.info(f"v5 STT speaker_id={sid!r} → {getattr(ev, 'transcript', '')[:60]!r}")
