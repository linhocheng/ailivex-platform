...
logger = logging.getLogger("ailivex-realtime-v12")
...
async def entrypoint(ctx: JobContext):
    ...
    # v12：讀網址工作臺。base_instructions = 開場 instructions；每讀一條網址就 append 進去（update_instructions）。
    ...
    # v12：註冊 RPC 'share_source' —— 前端同步框貼網址 → 角色暫停→讀→帶內容接話。
    ...
    try:
        ...
        logger.info("RPC 'share_source' registered (v12 讀網址工作臺)")
    except Exception as e:
