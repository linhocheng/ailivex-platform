...
async def entrypoint(ctx: JobContext):
    ...
    @function_tool(
        name="dispatch_task",
        description=(
            "派發背景任務給工廠執行，例如生成圖片或音檔。"
            "task_type 可以是：image_generation、audio_generation。"
            "intent 用一句話描述任務目的。params 依 task_type 傳入對應參數（JSON dict）。"
            "呼叫後系統背景執行，你只需口頭告知對方任務已安排，完成後會通知。"
        )
    )
    async def dispatch_task_tool(task_type: str, intent: str, params: str = "{}") -> str:
        ...
        try:
            ...
            messages = {
                "image_generation": "製圖任務已安排，完成後你可以在圖庫查看。",
                "audio_generation": "音檔生成任務已安排，完成後你可以在媒體庫查看。",
            }
            return messages.get(task_type, "任務已安排，完成後會通知你。")
        except Exception as e:
