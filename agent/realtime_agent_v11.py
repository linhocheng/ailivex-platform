"""
ailivex-realtime-agent v11 — v9 + 多人房三補強（回音過濾 / 講者身份 / 3a 收斂）
...
from agent.multi_party import (
    strip_speaker_prefix, normalize_for_echo, is_echo, format_recent, roster_summary, speaker_name,
)
...
class AilivexAgentV11(Agent):
    """v11 發言權控制 + 多人房補強（回音過濾 / 講者身份 / 3a 收斂在 entrypoint）。
    ...
    def __init__(self, *, agent_names: list[str], anthropic_key: str,
        ...
        self._vp_tap = None                  # v11 聲紋：tap（VP off → None，stt_node 直通＝v10）
        self._vp_session = None              # v11 聲紋：回音閘要讀 session.agent_state/current_speech

    def bind_vp(self, session, tap) -> None:
        """entrypoint 在 session 建好後綁定（REVIEW F：v10 從不用 self.session，得顯式綁進來）。"""
        self._vp_session = session
        self._vp_tap = tap

    async def stt_node(self, audio, model_settings=None):
        """v11 聲紋 tap：複製一份 STT 正在吃的 frame 餵聲紋，frame 照樣直通給 STT（diarization 不變）。

        對齊 LiveKit echo_transcriber 範例（包 audio iterable + return super().stt_node(...)）。
        VP off（tap=None）→ 直接 super，行為等同 v10。回音閘：角色說話時不餵 tap（第一安全護欄）。
        """
        tap = self._vp_tap
        if tap is None:
            return super().stt_node(audio, model_settings)
        sess = self._vp_session

        async def _tapped():
            async for frame in audio:
                speaking = sess is not None and (
                    getattr(sess, "current_speech", None) is not None
                    or getattr(sess, "agent_state", "") == "speaking"
                )
                if not speaking:
                    tap.push_frame(frame)
                yield frame

        return super().stt_node(_tapped(), model_settings)
    ...
    async def _floor_gate_llm(self, text: str) -> dict | None:
        ...
        try:
            ...
            logger.warning(f"v11 floor-gate LLM failed: {e}")
            return None
    ...
    def _remember(self, new_message, speaker: str, text: str) -> None:
        ...
        try:
            ...
            logger.warning(f"v11 remember chat_ctx failed: {e}")
        self._transcript.append({"role": "user", "content": text, "speaker": speaker, "ts": time.time()})
    ...
    async def on_user_turn_completed(self, turn_ctx, new_message) -> None:
        ...
        if is_echo(raw_text, self._recent_self_norms()):
            logger.info(f"v11 回音丟棄（speaker={speaker}）{text[:34]!r}")
            raise StopResponse
        ...
        if handoff:
            ...
            logger.info(f"v11 gate[{src}]：交棒第三方 → 讓位窗{YIELD_SECS:.0f}s（記住）{text[:40]!r}")
            raise StopResponse
        if addressed:
            self.yield_until = 0.0
            logger.info(f"v11 gate[{src}]：被點名 → 正常回話 {text[:44]!r}")
            return   # 框架會 commit → _on_item_added 那邊觸發判斷腦
        self._remember(new_message, speaker, text)
        if self.on_turn:
            self.on_turn()
        logger.info(f"v11 gate[{src}]：非對我（多人彼此聊）→ 靜默但記住 {text[:40]!r}")
        raise StopResponse
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info(f"Agent v11 initialized: soul={len(system_prompt)} chars, agent_names={agent_names}")
    ...
    # ── v11 聲紋（voiceprint）：tap STT 音訊 → 嵌入分群 → 後置覆寫講者（kill-switch: VP_ENABLED）──
    # 機制不丟 LLM：切語段/嵌入/分群/對齊都是確定性程式。VP off / 模型缺失 → 自然降級 v10。
    vp_tap = None
    if os.environ.get("VP_ENABLED", "0") == "1":
        try:
            from agent.voiceprint import VoiceprintEngine, OnlineClusterer
            from agent.audio_tap import VoiceprintTap
            try:
                _embedder = ctx.proc.userdata.get("vp_embedder")
            except Exception:
                _embedder = None
            _vp_engine = VoiceprintEngine(embedder=_embedder, clusterer=OnlineClusterer())
            _vp_state = {"last_spk": None}   # 即時「現在說話」去重

            def _vp_on_resolved(seg_ts: float, label: str, conf: float) -> None:
                # 後置覆寫（REVIEW #2/#3）：找 wall-clock 最近（≤2s）的 user 逐字稿，把 speaker 改成
                # VP 群 #N（VP 為真相，Soniox #N 降為提示）。format_recent/名冊照吃 #N，不必改 multi_party。
                best = None
                best_dt = 2.0
                for t in reversed(transcript):
                    if t.get("role") != "user" or t.get("ts") is None:
                        continue
                    dt = abs(t["ts"] - seg_ts)
                    if dt <= best_dt:
                        best, best_dt = t, dt
                if best is not None and best.get("speaker") != label:
                    logger.info(f"v11 VP 覆寫講者 {best.get('speaker')}→{label} "
                                f"(dt={best_dt:.2f}s conf={conf:.2f})")
                    best["speaker"] = label
                # 多人 latch 聯集：只會更敏感、不會比 v10 鈍（REVIEW：clusters>=2 OR _seen_speakers>=2）
                if _vp_engine.clusterer.num_clusters >= 2 and not ctx_flags["multi_person"]:
                    ctx_flags["multi_person"] = True
                    _cancel_timer()   # 比照 v10.0.2：偵測多人立刻停 3a（VP 是 v11 新增的多人觸發源）
                    logger.info(f"v11 VP：{_vp_engine.clusterer.num_clusters} 群 → 啟用多人 floor-gate，停止 3a")
                # 即時「現在說話」指示器（NET-NEW data-publish，REVIEW E：v10 沒有任何 publish_data）
                disp = speaker_name(label, roster)
                if disp and _vp_state.get("last_spk") != disp:
                    _vp_state["last_spk"] = disp

                    async def _pub(name=disp):
                        try:
                            await ctx.room.local_participant.publish_data(
                                json.dumps({"type": "current_speaker", "name": name}).encode("utf-8"),
                                reliable=True,
                            )
                        except Exception as e:
                            logger.warning(f"v11 publish current_speaker failed: {e}")

                    asyncio.create_task(_pub())

            vp_tap = VoiceprintTap(_vp_engine, on_resolved=_vp_on_resolved)
            agent.bind_vp(session, vp_tap)            # REVIEW F：顯式綁 session（回音閘要讀）
            asyncio.create_task(vp_tap.run())
            logger.info(f"v11 聲紋啟用 (embedder={'on' if _embedder else 'MISSING→log/降級'})")
        except Exception as e:
            logger.error(f"v11 聲紋初始化失敗 → 降級 v10: {e}")
            vp_tap = None
    ...
    async def _maybe_grab_floor():
        ...
        logger.info(f"v11 搶話! stance={stance} act={_inner['activation']:.2f} "
        ...
        try:
            ...
            logger.error(f"v11 搶話 generate_reply failed: {e}")
        # 搶完降溫，避免連環觸發
            ...
            try:
                ...
                resp = await client.messages.create(
                    ...
                    system=(
                        ...
                        '"want_to_speak":true|false,"what_to_say":"若想說，方向一句話否則空",'
                        '"names":{"#2":"名字"},"corrections":{"#2":"正確名字"}}\n'
                        ...
                        "names：只填你這次新對應上的『訪客標記→名字』，沒有就給 {}。\n"
                        "corrections：只在有人**糾正**身份/稱呼時填（如『我不是X，是Y』『他是Y不是X』）——"
                        "把該訪客標記對到**更正後**的名字；沒有就給 {}。" + name_task
                    ),
                ...
                try:
                    ...
                    for k, v in names.items():
                        ...
                        if k.startswith("#") and name and len(name) <= 12 and not roster.get(k):
                            roster[k] = name
                            logger.info(f"v11 名冊學到 {k}={name}")
                except Exception:
                    pass
                # 口頭糾正（REVIEW C：直接 parse raw——parse_inner_state 白名單會濾掉 corrections）。
                # corrections 凌駕一切：即使 roster 已有名字也覆寫（聲紋群早期可能先綁錯名）。
                try:
                    m2 = re.search(r"\{.*\}", raw, re.DOTALL)
                    corrections = (json.loads(m2.group(0)).get("corrections") or {}) if m2 else {}
                    for k, v in corrections.items():
                        k = str(k).strip()
                        name = re.sub(r"[（(].*?[)）]", "", str(v)).strip(" ？?…。.") if isinstance(v, str) else ""
                        if k.startswith("#") and name and len(name) <= 12 and roster.get(k) != name:
                            logger.info(f"v11 口頭糾正 → {k}={name}（覆寫舊={roster.get(k)!r}）")
                            roster[k] = name
                except Exception:
                    pass
                logger.info(f"v11 inner: stance={_inner['stance']} act={_inner['activation']:.2f} "
                            f"want={_inner['want_to_speak']} say={_inner['what_to_say'][:30]!r}")
            except Exception as e:
                logger.error(f"v11 inner judgment failed: {e}")
                return
        # 判斷完（鎖外）→ 檢查要不要搶話
    ...
    def _on_item_added(event):
        ...
        if role == "assistant":
            ...
            if is_echo(raw, _self_norms_recent()):   # 防禦：回音漏到這就跳過（一般已被 gate 丟）
                return
            transcript.append({"role": "user", "content": clean, "speaker": speaker, "ts": time.time()})
    ...
    def _on_user_transcribed(ev):
        if getattr(ev, "is_final", False):
            ...
            if is_echo(txt, _self_norms_recent()):
                logger.info(f"v11 STT 回音（不算講者）→ {txt[:40]!r}")
                return
            if sid is not None:
                _seen_speakers.add(sid)
            if (len(_seen_speakers) >= 2 or "（旁邊另一位" in txt) and not ctx_flags["multi_person"]:
                ctx_flags["multi_person"] = True
                _cancel_timer()   # 多人情境下 3a 不再跑，立刻停掉現有 timer（v10.0.2 port）
                logger.info(f"v11 偵測到多人情境 → 啟用 LLM floor-gate，停止 3a (speakers={_seen_speakers})")
            logger.info(f"v11 STT speaker_id={sid!r} → {txt[:60]!r}")

    async def _finalize(reason: str = "") -> None:
        _stopped["v"] = True   # ⑤ shutdown 也停 3a（涵蓋沒走 room disconnected 的關閉路徑）
        if vp_tap is not None:
            try:
                await vp_tap.aclose()   # v11 聲紋：收掉 tap 的 VAD stream + run loop
            except Exception:
                pass
        async with _finalize_lock:
            ...
            async def _do_lastsession():
                try:
                    ls = await asyncio.to_thread(extract_session_summary, transcript, _bu, _bs, _ak, roster)
                    if ls:
            ...
            async def _do_memories():
                try:
                    await asyncio.to_thread(
                        extract_and_save_memories,
                        user_id, character_id, char_name, transcript, _bu, _bs, _ak, roster,
                    )
                except Exception as e:
    ...
    def on_disconnected():
        ...
        if vp_tap is not None:
            try:
                asyncio.create_task(vp_tap.aclose())   # v11 聲紋：斷線即收 tap
            except Exception:
                pass
    ...
    async def _maybe_interject():
        ...
        if _stopped["v"]:   # ⑤ 斷線/收尾 → 不再重排，3a 迴圈到此終止
            return
        # v11 讓位窗：剛把棒子交給第三方 → 3a 閉嘴，讓那個人講（不報幕）
        ...
        #    多人時 AI 只靠 gate（被點名才回）+ inner（有貨才主動）。（v10.0.2 port）
