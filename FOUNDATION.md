...
| 3 | 安全與威脅防禦 | 已灌（CSP 嚴格版待做） | secrets 進 Secret Manager；SSRF guard（url-reader）；prompt injection 靠 R12-14 紀律；security headers（v18.15）；**資安掃描四件套 CI 已接（v18.19，.github/workflows/security.yml：gitleaks/Semgrep+p/python/npm audit 每 push＋ZAP weekly；actions pin SHA）**；CSP 保守版待升 nonce（債 D6） |
